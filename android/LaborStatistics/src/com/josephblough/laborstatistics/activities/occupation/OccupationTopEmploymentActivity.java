...
public class OccupationTopEmploymentActivity extends ListActivity implements OnItemSelectedListener {
	...
	setContentView(R.layout.occupation_top_employment);

	topItemSpinner = 
		(Spinner)findViewById(R.id.occupation_top_employment_items_spinner);
	...
	((TextView)findViewById(R.id.occupation_name_top_employment_label)).setText(occupation.occupationName);
    }
	...
	new TopIndustriesRetrieverTask(DataTransport.EMPLOYMENT_CRITERIA, new SalaryDisplayCallback()).execute(app.selectedOccupation);
    }
	...
	new TopLocationsRetrieverTask(DataTransport.STATE_AREA_TYPE, DataTransport.EMPLOYMENT_CRITERIA, 
	...
	new TopLocationsRetrieverTask(DataTransport.MUNICIPALITY_AREA_TYPE, DataTransport.EMPLOYMENT_CRITERIA, 
		new SalaryDisplayCallback()).execute(app.selectedOccupation);
	...
	public void success(List<DataDataset> items) {
		...
		if (name != null) {
		    ...
		    final String value = formatter.format(Double.valueOf(item.value));
	    ...
	    SimpleAdapter adapter = new SimpleAdapter(
		    OccupationTopEmploymentActivity.this, data, 
	    ...
	    if (items.size() == 0)
		Toast.makeText(OccupationTopEmploymentActivity.this, R.string.no_data, Toast.LENGTH_LONG).show();
	}
	...
	public void error(String error) {
	    ...
	    Toast.makeText(OccupationTopEmploymentActivity.this, error, Toast.LENGTH_LONG).show();
	}
	...
	private String getName(final DataDataset data) {
		...
		final IndustryDataset industry = DataLibrary.getIndustry(
			OccupationTopEmploymentActivity.this, industryCode);
		if (industry != null)
		    return industry.industryName;
	    }
	    else {
		final String areaCode = DataRetriever.extractAreaCodeFromSeriesId(data.seriesId);
		final AreaDataset area = DataLibrary.getArea(OccupationTopEmploymentActivity.this, areaCode);
		if (area != null)
